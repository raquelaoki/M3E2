...
class HiCI():
    ...
    def fit(self, type_target, batch_size=128, total_epochs=100, lr=0.01, lr_decay=0.6, ite_per_decay=1, weight_decay=0.0,
        ...
        for epoch in range(total_epochs):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                optimizer.zero_grad()
                encoded, decoded, pred_y = model(causal, noncausal)
...
class autoencoder(nn.Module):
    def __init__(self, num_covs, num_feats,
                 enc_layers=4, enc_nodes=[64, 32, 16], dec_layers=4, dec_nodes=[16, 32, 64]):
...
class L_21(nn.Module):
    ...
    def forward(self, phi_x, t):
        ...
        # ---- prob: if not every treatment shows up in a batch ----
        # ---- sol: ignore the missing treatments ----
        valid_K = 0
        for i in range(K):
            phi_i = phi_x[t[:, i] == 1]
            if phi_i.shape[0] > 0:
...
# total loss for continuous outcome
