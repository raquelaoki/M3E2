...
class copula_simulated_data(object):
    ...
    def __init__(self,k = 4, s = 1, B = [2,0.5, -0.4, 0.2], gamma = 2.8, sigma2_t =1, sigma2_y = 1, tau_l = [3, -1, 1, -0.06], tau_nl = [-4],n = 8000 ):
    ...
    def get_data(self):
        u = np.random.normal(loc = 0, scale = 1 , size = self.n*self.s).reshape(self.n,self.s) #mu = rep(0, s), Sigma = diag(s)
        if self.s > 1: 
        ...
        y_continuous = self.g_yt(tr, self.tau_l, self.tau_nl)+(u1*self.gamma).reshape(self.n,)+ np.random.normal(loc = 0, scale = self.sigma2_y, size = self.n)
    ...
    def get_true_coefs(self):
        aux1 = np.linalg.solve(np.array(self.B).reshape(self.k,1)*(np.transpose(self.B).reshape(1,self.k)) + self.sigma2_t*np.identity(self.k),np.identity(self.k))
        ...
        #5 x 4 , 4 x 1 , 1 x 1
        ytilde_mean_do_bias = np.array((np.concatenate([t_choice,t2], axis = 0).dot(np.transpose(coef_mu_u_t)))*self.gamma)
